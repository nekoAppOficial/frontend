    ...
    props.socket.on(`acceptFriend`, (friend) => {
      const InterVal = setInterval(() => {
        if (myFriends.length > 0) {
          ...
          newFriends.map((f, index) => {
            if (f.id == friend.id) {
              newFriends[index].statusAmizade = `accept`;
              newFriends[index].photo = friend.photo;
              newFriends[index].coverPhoto = friend.coverPhoto;
              newFriends[index].backgroundColor = friend.backgroundColor;
              newFriends[index].aboutMe = friend.aboutMe;
      ...
      <div>
        <h2 className="title-x4dI75 container-q97qHp">
          {props.typeFriend == `accept` ? `ACEITOS` : `PENDENTES`}—{" "}
          {
            myFriends.filter(
              (friend) =>
                friend.statusAmizade === props.typeFriend &&
                friend.username.includes(filterFriends)
            ).length
          }
        </h2>
      </div>
          ...
          <div className="userInfo-2WpsYG">
                ...
                >
                  <div className="avatarStack-3vfSFa">
                    <img
                      ...
                      src={`${
                        typeof props.friend.photo != `object`
                          ? props.friend.photo
                          : window[`getPath`]() + `assets/default.webp`
                      }`}
                      alt=" "
            ...
            <div className="text-3j8t_e">
              <div className="discordTag-3HiQI9 nameTag-H6kSJ0">
                <span className="username-3JLfHz username-Qpc78p">
              ...
              <div className="subtext-xfubwR">
                {props.friend.statusAmizade == `accept` && (
                  <div className="text-MPIeXO">
                    {props.friend.status == `offline` ? `Offline` : `Online`}
                  </div>
                )}
                {props.friend.statusAmizade !== `accept` && (
                  <div className="subtext-xfubwR">
                ...
                <div
                  ...
                  onMouseOver={(e) => {
                    if (!props.toolTipBottom) {
                      props.toolTipShowBottom(
                        `Aceitar`,
                        acceptFriendButton.current.offsetLeft + 304,
                        acceptFriendButton.current.offsetTop + 130
                      );
                    }
                  }}
                  onMouseLeave={(e) => {
              ...
              <div
                ...
                onMouseOver={(e) => {
                  if (!props.toolTipBottom) {
                    props.toolTipShowBottom(
                      `Recusar`,
                      recuseFriendButton.current.offsetLeft + 300,
                      recuseFriendButton.current.offsetTop + 130
                    );
                  }
                }}
                onMouseLeave={(e) => {
              ...
              <Link
                ...
                onMouseOver={(e) => {
                  if (!props.toolTipBottom) {
                    props.toolTipShowBottom(
                      `Mensagem`,
                      chatFriendButton.current.offsetLeft + 293,
                      chatFriendButton.current.offsetTop + 130
                    );
                  }
                }}
                onMouseLeave={(e) => {
              ...
              <div
                ...
                onMouseOver={(e) => {
                  if (!props.toolTipBottom) {
                    props.toolTipShowBottom(
                      `Desfazer Amizade`,
                      recuseFriendButton.current.offsetLeft + 250,
                      recuseFriendButton.current.offsetTop + 130
                    );
                  }
                }}
...
const NoHaveFriends = () => {
  return (
    <div className="emptyStateContainer-1NHrfT">
        ...
        >
          <div
            ...
            style={{
              ...
              height: "218px",
              backgroundImage:
                'url("../assets/b36de980b174d7b798c89f35c116e5c6.svg")',
            }}
          />
          <div
          ...
          >
            <div className="text-27cdrj marginTop8-24uXGp">
              Ninguém por perto para brincar com o Wumpus.
            </div>
          </div>
        </div>
      </div>
